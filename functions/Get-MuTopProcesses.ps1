...
#>
  ...
  $AllProcesses = Get-CimInstance Win32_PerfFormattedData_PerfProc_Process -ComputerName $ComputerName |
...
set-alias gtp get-MuTopProcesses

function show-MuTopProcesses {
<#
.SYNOPSIS
...
#>
  ...
  get-MuTopProcesses $ComputerName $SortOrder $NumberOfProcesses | ft -a

  write-debug "$(get-date -format 'hh:mm:ss.ffff') Function end: $([string]$MyInvocation.Line) "

}

set-alias stp show-MuTopProcesses
set-alias top show-MuTopProcesses
