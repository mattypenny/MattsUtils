...
#>
        ...
        $options = @(
            (
                Import-Csv "$ENV:PSParametersFolder\GeneralParameters.csv" |
                    Where-Object Program -EQ 'Get-MtpAdUser'
            ).Value
        )
    ...
    process {
        ...
        switch ($OUShortName) {
            'Disabled' {
                $Ou = 'OU=Disabled Accounts,DC=simplyhealth,DC=co,DC=uk'
            }
            'Leavers' {
                $Ou = 'OU=Leavers,OU=Disabled Accounts,DC=simplyhealth,DC=co,DC=uk'
            }
            'LeaversPendingDeletion' {
                $Ou = 'OU=Leavers Pending Deletion,OU=Leavers,OU=Disabled Accounts,DC=simplyhealth,DC=co,DC=uk'
            }
            'Investigate' {
                $Ou = 'OU=Investigate,OU=Disabled Accounts,DC=simplyhealth,DC=co,DC=uk'
            }
            'Mat' {
                $Ou = 'OU=Mat or Gardening leave and Long Term Sick,OU=Disabled Accounts,DC=simplyhealth,DC=co,DC=uk'
            }
            'Employees' {
                $Ou = 'OU=Employees,DC=simplyhealth,DC=co,DC=uk'
            }
        }
...
Set-Alias Get-MadUser Get-MtpAdUser
Set-Alias gmu Get-MtpAdUser
Set-Alias gmau Get-MtpAdUser
Set-Alias gau Get-MtpAdUser
